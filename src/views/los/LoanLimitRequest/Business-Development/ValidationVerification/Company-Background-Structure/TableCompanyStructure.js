...
import { openUrl, searchKeyObjectInArray } from '../../../../../../utility/Utils'

function TableCompanyStructure({ data = [], handleTrash }) {
    const MAX = 3;

    const deleteData = (val, activeStatus) => {
        const form = new FormData();
        form.set('merchantStructureId', val.id);
        form.set('loanLimitRequestId', store.getState().customer.loanLimitRequestId);
        ...
        let ktpFile = searchKeyObjectInArray(store.getState().upload.UploadDoc, "inputName", val?.structureGroup + "_ktpFile")?.["fileUrl"];
        let npwpFile = searchKeyObjectInArray(store.getState().upload.UploadDoc, "inputName", val?.structureGroup + "_npwpFile")?.["fileUrl"];
...
export default TableCompanyStructure